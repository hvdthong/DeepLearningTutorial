...
class TextCNN(object):
    ...
    def __init__(
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.device("/cpu:" + str(filter_size)):
                with tf.name_scope("conv-maxpool-%s" % filter_size):
                    ...
                    conv_bn = batch_norm(conv, center=True, scale=True, is_training=self.phase)
        ...
        # Add dropout
        with tf.device('/gpu:6'), tf.name_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.h_pool_flat, self.dropout_keep_prob)

        # Final (unnormalized) scores and predictions
        with tf.device('/gpu:6'), tf.name_scope("output"):
            W = tf.get_variable(
                name="W_output",
        ...
        with tf.device('/gpu:6'), tf.name_scope("loss"):
            losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.scores, labels=self.input_y)
            self.loss = tf.reduce_mean(losses) + l2_reg_lambda * l2_loss

        # Accuracy
        with tf.device('/gpu:6'), tf.name_scope("accuracy"):
